...
@section('title',  trans('lang.uc_main_heading')  )
...
@section('breadcrumb-title')
    <h3>{{ trans('lang.uc_main_heading') }}</h3>
@endsection

@section('breadcrumb-items')
    <li class="breadcrumb-item active">{{ trans('lang.uc_main_heading') }}</li>
@endsection
            ...
            <div class="col-sm-12">
                <div class="card">
                    ...
                    <div class="card-header project-list">
                        <h5>{{ trans('lang.uc_dt_heading') }}</h5>
                        <span>{{ trans('lang.uc_dt_paragraph') }}</span>
                        @if(isset($data['permission'][0]->CanAdd) && $data['permission'][0]->CanAdd == 1)
                            <span><a class="btn btn-primary addbtn" href="javascript:void(0)"
                                     data-uk-modal="{target:'#addModal'}" id="add"> <i data-feather="plus-square"> </i>Create New UC</a></span>
                        @endif
                        ...
                        <div class="table-responsive2">
                            <table class="display datatables" id="datatable_custom">
                                ...
                                @foreach ($data['data'] as $keys=>$value)
                                    <tr data-id="{{ $value->id }}">
                                        <td>{{$sno++}}</td>
                                        <td>{{(isset($value->uccode) && $value->uccode!=''?$value->uccode:'')}}</td>
                                        ...
                                        <td data-id="{{ $value->id }}"
                                            data-uccode="{{ $value->uccode }}"
                                            data-ucname="{{ $value->ucname }}"
                                            data-distname="{{ $value->distname }}"
                                            data-provname="{{ $value->provname }}">
                                ...
                                <div class="modal-dialog" role="document">
                                    <div class="modal-content ">
                                        ...
                                        <div class="modal-header bg-primary white">
                                            <h4 class="modal-title white" id="myModalLabel_add">Add UC</h4>
                                        ...
                                        <div class="modal-body  ">
                                            ...
                                            <div class="mb-1 form-group">
                                                <label class="col-form-label" for="ucname">UC Name: </label>
                                                <input type="text" class="form-control ucname"
                                                       id="ucname" required>
                                            </div>
                                        </div>
                            ...
                            <div class="modal fade text-left" id="editModal" tabindex="-1" role="dialog"
                                ...
                                <div class="modal-dialog" role="document">
                                    <div class="modal-content ">
                                        ...
                                        <div class="modal-header bg-primary white">
                                            <h4 class="modal-title white" id="myModalLabel_edit">Edit UC</h4>
                                        ...
                                        <div class="modal-body  ">
                                            <div class="mb-1 form-group">
                                                ...
                                                <select class="form-control col-sm-12" id="province_edit" required onchange="changeProvince('province_edit','district_edit')">
                                                    <option value="0">Select Province</option>
                                                    @if (isset($data['province']) && $data['province'] != '')
                                                        @foreach ($data['province'] as $keys=>$d)
                                                            <option
                                            ...
                                            <div class="mb-1 form-group">
                                                <label class="col-form-label" for="ucname_edit">UC Name: </label>
                                                <input type="text" class="form-control ucname_edit"
                                                       id="ucname_edit" required>
                                            </div>
                                        </div>
                                        <div class="modal-footer">
                                            <button type="button" class="btn grey btn-secondary" data-bs-dismiss="modal"
                            ...
                            <div class="modal fade text-left" id="deleteModal" tabindex="-1" role="dialog"
                                ...
                                <div class="modal-dialog" role="document">
                                    <div class="modal-content ">
                                        ...
                                        <div class="modal-header bg-primary white">
                                            <h4 class="modal-title white" id="myModalLabel_delete">Delete UC</h4>
...
@section('script')
    ...
    <script>
        ...
        function addData() {
            $('#province').css('border', '1px solid #babfc7');
            $('#district').css('border', '1px solid #babfc7');
            $('#ucname').css('border', '1px solid #babfc7');
            var flag = 0;
            var data = {};
            data['provcode'] = $('#province').val();
            data['provname'] = $('#province').find('option:selected').attr('data-provname');
            data['distcode'] = $('#district').val();
            data['distname'] = $('#district').find('option:selected').attr('data-district');
            data['ucname'] = $('#ucname').val();
            if (data['provcode'] == '' || data['provcode'] == undefined || data['provcode'] == 0) {
                $('#province').css('border', '1px solid red');
                flag = 1;
                toastMsg('Province', 'Invalid Province', 'danger');
                return false;
            }
            ...
            if (data['ucname'] == '' || data['ucname'] == undefined || data['ucname'] == 0) {
                $('#ucname').css('border', '1px solid red');
                flag = 1;
                toastMsg('UC Name', 'Invalid UC Name', 'danger');
                return false;
            }
            if (flag == 0) {
                showloader();
                CallAjax('{{ url('/Uc/addUc') }}', data, 'POST', function (result) {
                    hideloader();
        ...
        function getEdit(obj) {
            ...
            if (data['id'] != '' && data['id'] != undefined) {
                CallAjax('{{ url('/Uc/detail/') }}', data, 'GET', function (result) {
                    if (result != '' && JSON.parse(result).length > 0) {
        ...
        function editData() {
            ...
            data['provcode'] = $('#province_edit').val();
            data['provname'] = $('#province_edit').find('option:selected').attr('data-provname');
            data['distcode'] = $('#district_edit').val();
            data['distname'] = $('#district_edit').find('option:selected').attr('data-district');
            data['ucname'] = $('#ucname_edit').val();
            if (data['id'] == '' || data['id'] == undefined) {
            ...
            if (data['provcode'] == '' || data['provcode'] == undefined || data['provcode'] == 0) {
                $('#province_edit').css('border', '1px solid red');
                flag = 1;
                toastMsg('Province', 'Invalid Province', 'danger');
                return false;
            }
            if (data['distcode'] == '' || data['distcode'] == undefined || data['distcode'] == 0) {
                $('#distcode_edit').css('border', '1px solid red');
                flag = 1;
                toastMsg('District', 'Invalid District', 'danger');
                return false;
            }
            if (data['ucname'] == '' || data['ucname'] == undefined || data['ucname'] == 0) {
                $('#ucname_edit').css('border', '1px solid red');
                flag = 1;
                toastMsg('UC Name', 'Invalid UC Name', 'danger');
                return false;
            }
            if (flag === 0) {
                showloader();
                CallAjax('{{ url('/Uc/editUc') }}', data, 'POST', function (result) {
                    hideloader();
                    if (result !== '' && JSON.parse(result).length > 0) {
                        var response = JSON.parse(result);
                        try {
        ...
        function deleteData() {
            ...
            if (data['id'] == '' || data['id'] == undefined) {
                ...
                showloader();
                CallAjax('{{ url('/Uc/deleteUc') }}', data, 'POST', function (result) {
                    hideloader();
                    if (result !== '' && JSON.parse(result).length > 0) {
                        var response = JSON.parse(result);
                        try {
