...
use App\Models\Patient_visit_report_Model;
use App\Models\Settings_Model;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Auth;

class Patient_visit_report extends Controller
{
    ...
    public function index()
    {
        ...
        $data['permission'] = Settings_Model::getUserRights(Auth::user()->idGroup, '', 'patient_visit_report');
        /*==========Log=============*/
        $trackarray = array(
            "activityName" => "Patient_visit_report",
            "action" => "View Patient_visit_report -> Function: Patient_visit_report/index()",
            "PostData" => "",
            "affectedKey" => "",
            "idUser" => Auth::user()->id,
        ...
        if ($data['permission'][0]->CanView == 1) {
            ...
            $data["getData"] = Patient_visit_report_Model::getData($searchFilter);

            $trackarray["mainResult"] = "Success";
            $trackarray["result"] = "View Success";
            Custom_Model::trackLogs($trackarray, "all_logs");
            return view('patient_visit_report', ['data' => $data]);
        } else {
