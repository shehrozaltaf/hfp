...
use App\Models\Presenting_Complaint_Model;
use App\Models\Settings_Model;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Auth;

class Presenting_Complaint extends Controller
{
    ...
    public function index()
    {
        ...
        $data['permission'] = Settings_Model::getUserRights(Auth::user()->idGroup, '', 'presenting_complaint');
        /*==========Log=============*/
        $trackarray = array(
            "activityName" => "Presenting_Complaint",
            "action" => "Presenting_Complaint -> Function: Presenting_Complaint/index()",
            "PostData" => "",
            "affectedKey" => "",
            "idUser" => Auth::user()->id,
        ...
        if ($data['permission'][0]->CanView == 1) {
            ...
            $data["getData"] = Presenting_Complaint_Model::getData($searchFilter);

            $trackarray["mainResult"] = "Success";
            $trackarray["result"] = "View Success";
            Custom_Model::trackLogs($trackarray, "all_logs");
            return view('Presenting_Complaint', ['data' => $data]);
        } else {
